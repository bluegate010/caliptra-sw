...
use caliptra_runtime::{
    hash_chain::HashChain, mailbox::Mailbox, ContextState, DpeInstance, Drivers, Hmac,
...
pub fn handle_command(drivers: &mut Drivers) -> CaliptraResult<MboxStatusE> {
    loop {
        ...
        match cmd {
            ...
            // Computes a digest from the hash chain for a given target SVN.
            CommandId(OPCODE_READ_HASH_CHAIN_DIGEST) => {
                let target_svn = u32::read_from(read_request(&drivers.mbox)).unwrap();

                HashChain::derive_secret(drivers, target_svn, b"", KEY_ID_TMP)?;
