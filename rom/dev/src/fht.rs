...
use caliptra_common::{
    keyids::{KEY_ID_FMC_ECDSA_PRIV_KEY, KEY_ID_FW_HASH_CHAIN, KEY_ID_ROM_FMC_CDI},
...
impl FhtDataStore {
    ...
    /// The firmware hash chain is stored in a KeyVault slot.
    pub const fn fw_hash_chain_store() -> HandOffDataHandle {
        HandOffDataHandle(((Vault::KeyVault as u32) << 12) | KEY_ID_FW_HASH_CHAIN as u32)
    }
...
pub fn initialize_fht(env: &mut RomEnv) {
    ...
    env.persistent_data.get_mut().fht = FirmwareHandoffTable {
        ...
        rt_hash_chain_max_svn: MAX_RUNTIME_SVN as u16,
        rt_hash_chain_kv_hdl: FhtDataStore::fw_hash_chain_store(),
        ..Default::default()
    };
}
