...
impl FhtDataStore {
    ...
    pub const fn deprecated_fmc_svn_store() -> HandOffDataHandle {
        HandOffDataHandle(
            ((Vault::DataVault as u32) << 12)
                | (DataVaultRegister::Sticky32BitReg as u32) << 8
                | ColdResetEntry4::DeprecatedFmcSvn as u32,
        )
    }
...
pub fn initialize_fht(env: &mut RomEnv) {
    ...
    env.persistent_data.get_mut().fht = FirmwareHandoffTable {
        ...
        deprecated_fmc_svn_dv_hdl: FhtDataStore::deprecated_fmc_svn_store(),
