...
impl FirmwareProcessor {
    pub fn process(env: &mut RomEnv) -> CaliptraResult<FwProcInfo> {
        ...
        Self::populate_fw_hash_chain(env)?;
    ...
    #[cfg_attr(not(feature = "no-cfi"), cfi_impl_fn)]
    fn populate_fw_hash_chain(env: &mut RomEnv) -> CaliptraResult<()> {
        let epoch = env.persistent_data.get().manifest1.header.owner_data.epoch;
        let svn = env.data_vault.rt_svn();
        ...
        hash_chain::initialize_hash_chain(env, epoch, chain_len)?;

        cprintln!("[fwproc] Chain initialized");

        report_boot_status(FwProcessorCalculateHashChainComplete.into());
